...
Known, NOT fully resolved by this pass: JLine's own system-terminal detection falls back to a "dumb terminal" (plain buffered line reading, no arrow-key history at all) in constrained/headless pty environments — reproduced consistently in this project's own agentic sandbox across three different pty-construction methods (`subprocess.Popen` handed a raw pty pair, the same with explicit JVM opts, and a proper `pty.fork()`-based controlling terminal), even though `TERM` was set and `stty`/`tput` were both present. The exact underlying cause was not tracked down (attempts to get JLine's own debug-level log output hung or produced no further output, not investigated past that point) — genuinely unresolved, not silently declared fixed. Ordinary line execution and the REPL's own read/eval/print cycle still work correctly even under the dumb-terminal fallback, only the interactive history/editing features are unavailable there. Real up/down-arrow recall in an actual user terminal (a real terminal emulator, not a sandboxed/headless pty) was NOT independently confirmed working end-to-end in this session — needs the user's own real terminal to fully verify.
