...
(defn vocab []
  (merge
    (builtin "indispensability" (fn [ctx] (push! ctx (indisp/indispensability (pop-val! ctx))))
             "( subdivisions -- ranks )" "Barlow indispensability rank per pulse, downbeat always highest")
    (builtin "normalize-weights" (fn [ctx] (push! ctx (indisp/normalize-weights (pop-val! ctx))))
             "( weights -- weights' )" "divides weights by their own max, landing them in [0,1]")
    (builtin "normalized-indispensability" (fn [ctx] (push! ctx (indisp/normalized-indispensability (pop-val! ctx))))
             "( subdivisions -- weights )" "indispensability + normalize-weights in one step")
    (builtin "tilt-probabilities" (fn [ctx] (let [adherence (pop-val! ctx) psi-vals (pop-val! ctx)]
    ...
    (builtin "power-law-probabilities" (fn [ctx] (let [adherence (pop-val! ctx) psi-vals (pop-val! ctx)]
    ...
    (builtin "density-grid" (fn [ctx] (let [density (pop-val! ctx) ranks (pop-val! ctx)]
