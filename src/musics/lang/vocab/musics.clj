...
(defn vocab []
  (merge
    ...
    (builtin "find" (fn [ctx] (push! ctx (m/find (->kw (pop! ctx))))) "( id -- node/f )" "looks up a node by id in the repo")
    ...
    (builtin "children" (fn [ctx] (push! ctx (m/children (->kw (pop! ctx))))) "( id -- children )" "a container's own immediate children")
    (builtin "leaves" (fn [ctx] (push! ctx (m/leaves (->kw (pop! ctx))))) "( id -- leaves )" "every leaf note/rest under an id, in order")
    (builtin "sq" (fn [ctx] (push! ctx (m/sq (->kw (pop! ctx))))) "( id -- seq )" "a container's own children as a bare, playable seq")
    (builtin "inspect" (fn [ctx] (m/inspect (->kw (pop! ctx)))) "( id -- )" "prints a node's own structure")
    (builtin "inspect-all" (fn [_ctx] (m/inspect)) "( -- )" "prints a session-wide node-count overview")
    (builtin "ctx" (fn [ctx] (m/ctx (->kw (pop! ctx)))) "( id -- )" "prints a part's own context chain")
    (builtin "ctx-value" (fn [ctx] (let [time (pop! ctx) key (->kw (pop! ctx)) id (->kw (pop! ctx))]
                                       (push! ctx (m/ctx-value id key time))))
             "( id key time -- value )" "samples one context key's own value at a given time")
    (builtin "locate" (fn [ctx] (let [path (pop! ctx) id (->kw (pop! ctx))]
                                    (push! ctx (m/locate id path))))
             "( id path -- node )" "navigates from id along an explicit selector path")
    (builtin "describe" (fn [ctx] (push! ctx (m/describe (->kw (pop! ctx))))) "( id -- str )" "a human-readable description of a node")
    (builtin "print-structure" (fn [ctx] (m/print-structure (->kw (pop! ctx)))) "( id -- )" "prints a node's own full tree structure")
    (builtin "expand" (fn [ctx] (push! ctx (m/expand (pop! ctx)))) "( leaf -- path )" "finds where a real leaf value sits in the repo tree")
    ...
    (builtin "warm-up-n!" (fn [ctx] (let [ms (pop! ctx) n (pop! ctx)] (m/warm-up! n ms))) "( n ms -- )" "warm-up!, n times, ms apart")
    (builtin "disconnect" (fn [_ctx] (m/disconnect)) "( -- )" "closes the MIDI connection")
    (builtin "play" (fn [ctx] (m/play (->kw (pop! ctx)))) "( id -- )" "flushes every voice, then plays id")
    (builtin "play-add" (fn [ctx] (push! ctx (m/play-add (->kw (pop! ctx))))) "( id -- path )" "plays id alongside whatever's already playing")
    (builtin "play-change" (fn [ctx] (let [arg (->kw (pop! ctx)) path (->kw (pop! ctx))]
                                         (push! ctx (m/play-change path arg))))
             "( path id -- path )" "replaces one already-playing track's own material")
    (builtin "voice-at" (fn [ctx] (push! ctx (m/voice-at (->kw (pop! ctx))))) "( path -- voice )" "the live voice map at a given track path")
    (builtin "play-file!" (fn [ctx] (m/play-file! (pop! ctx))) "( path -- )" "parses, commits, and plays a .mus file in one step")
    (builtin "display" (fn [ctx] (push! ctx (m/display (->kw (pop! ctx))))) "( id -- )" "a synchronous, silent preview of what play would do")
    ...
    (builtin "play!" (fn [ctx] (let [v (pop! ctx)
                                       {:keys [ids]} (if (string? v) (m/parse v) v)]
                                   (m/play (vec ids))))
             "( text/{:ids ids} -- )" "parses (if needed), commits, and plays in one step")
    (builtin "p!" (fn [ctx] (m/p! (pop! ctx))) "( text -- )" "musics.core/play!'s own short name")

    ;; -- generative transforms -------------------------------------------
    (builtin "times" (fn [ctx] (let [material (pop! ctx) n (pop! ctx)]
                                   (push! ctx (m/times n material))))
             "( n material -- material' )" "repeats material n times")
    (builtin "transpose" (fn [ctx] (let [material (pop! ctx) semitones (pop! ctx)]
                                       (push! ctx (m/transpose semitones material))))
             "( semitones material -- material' )" "shifts every pitch by a fixed number of semitones")
    (builtin "invert" (fn [ctx] (let [material (pop! ctx) axis (pop! ctx)]
                                    (push! ctx (m/invert axis material))))
             "( axis material -- material' )" "mirrors every pitch around an axis")
    (builtin "invert-mean" (fn [ctx] (push! ctx (m/invert (pop! ctx)))) "( material -- material' )" "invert, axis defaulted to the material's own mean pitch")
    (builtin "scale" (fn [ctx] (let [material (pop! ctx) factor (pop! ctx)]
                                   (push! ctx (m/scale factor material))))
             "( factor material -- material' )" "scales every duration by a fixed factor")
    (builtin "reverse" (fn [ctx] (push! ctx (m/reverse (pop! ctx)))) "( material -- material' )" "reverses material's own order")
    (builtin "shuffle" (fn [ctx] (push! ctx (m/shuffle (pop! ctx)))) "( material -- material' )" "randomly reorders material")
    (builtin "thread" (fn [ctx] (let [material (pop! ctx) f (callable->fn ctx (pop! ctx))]
                                    (push! ctx (m/thread f material))))
             "( f material -- material' )" "applies f to every element of material")
    (builtin "active-key" (fn [ctx] (push! ctx (m/active-key (->kw (pop! ctx))))) "( id -- ks )" "the key currently in scope for a part")
    (builtin "tonal-transpose" (fn [ctx] (let [material (pop! ctx) steps (pop! ctx) ks (pop! ctx)]
    ...
    ;; -- persistence --------------------------------------------------------
    (builtin "write" (fn [ctx] (m/write (pop! ctx))) "( path -- )" "saves the whole current repo to a file")
    (builtin "load" (fn [ctx] (m/load (pop! ctx))) "( path -- )" "replaces the current repo with a saved file's own content")
    (builtin "ly-to-mus" (fn [ctx] (push! ctx (m/ly-to-mus (pop! ctx)))) "( path -- mus-path )" "converts a LilyPond file to a sibling .mus file")
    ...
    (builtin "help?" (fn [ctx] (m/help (pop! ctx))) "( key -- )" "prints musics.core's own help for one context key")

    ;; -- action registry / schedule -------------------------------------------
    (builtin "register-action!" (fn [ctx] (let [f (callable->fn ctx (pop! ctx)) id (->kw (pop! ctx))]
                                              (m/register-action! id f)))
             "( id f -- )" "parks a callable action under id, for trigger!/schedule!")
    (builtin "unregister-action!" (fn [ctx] (m/unregister-action! (->kw (pop! ctx)))) "( id -- )" "removes a registered action")
    (builtin "trigger!" (fn [ctx] (let [arg (pop! ctx) id (->kw (pop! ctx))]
                                      (push! ctx (m/trigger! id arg))))
             "( id arg -- result )" "runs a registered action directly, right now")
    (builtin "schedule!" (fn [ctx] (let [action-id (->kw (pop! ctx)) phase (->kw (pop! ctx)) id (->kw (pop! ctx))]
                                       (m/schedule! id phase action-id)))
             "( id phase action-id -- )" "arms a one-shot action for the next [id phase] boundary")
    (builtin "unschedule!" (fn [ctx] (let [phase (->kw (pop! ctx)) id (->kw (pop! ctx))]
    ...
    (builtin "scheduled?" (fn [ctx] (let [phase (->kw (pop! ctx)) id (->kw (pop! ctx))]
    ...
    (builtin "scheduled-repeating?" (fn [ctx] (let [phase (->kw (pop! ctx)) id (->kw (pop! ctx))]
                                                  (push! ctx (m/scheduled-repeating id phase))))
             "( id phase -- entry/f )" "checks one specific repeating schedule slot")
    (builtin "unschedule-repeating!" (fn [ctx] (let [phase (->kw (pop! ctx)) id (->kw (pop! ctx))]
                                                   (m/unschedule-repeating! id phase)))
             "( id phase -- )" "cancels a repeating schedule-tx! entry")
    (builtin "schedule-tx!" (fn [ctx] (let [phase (->kw (pop! ctx)) id (->kw (pop! ctx))]
                                          (push! ctx (m/schedule-tx! id phase))))
             "( id phase -- )" "redirects one voice's own :view to whatever's newly committed, next boundary")

    ;; -- misc / state ---------------------------------------------------
    (builtin "music-eval" (fn [ctx] (push! ctx (m/music-eval (pop! ctx)))) "( text -- result )" "mu!'s own :eval hook, callable directly")
