(ns input.reader.radical-tree-walker
  "Post-parse tree walker for radical.ebnf -- a standalone adaptation of
   input.reader.flat-tree-walker (musics.ebnf's own walker), NOT a
   shared-dispatch refactor of it: that file is untouched by this one,
   deliberately (see the project root CLAUDE.md / this branch's own
   history for why -- radical.ebnf is still an experimental, unmerged
   second grammar, not yet worth risking the production walker for).

   radical.ebnf keeps every leaf/instruction/variable rule TAG-
   IDENTICAL to musics.ebnf (Note, Pitch, Assignment, VarDef, BangConst,
   Ramp, etc. are byte-for-byte the same rules, just different
   container brackets around them) -- so every leaf/instruction/
   variable-level function below is a straight, unmodified copy.
   What's actually different from flat-tree-walker:
     - :Unit/:AtomicAlgo/:ElementAlgo/:algo/:Time/:Tempo/:Key have no
       case here at all -- radical.ebnf produces none of those tags,
       Unit was dropped from the grammar entirely and the rest are
       gone along with @[ ]/@{ } and \\time/\\tempo/\\key.
     - walk-repeat absorbs what used to be a separate walk-tremolo --
       radical.ebnf's own repeat rule covers unfold/volta/tremolo as
       one rule (a third repeat-type value), not two rules sharing a
       keyword the way musics.ebnf's repeat/tremolo still do. Field
       names differ too: radical.ebnf's repeat uses `count`/
       `alternative` where musics.ebnf's used `repeats`/`volta`.
     - walk-times/walk-tuplet/walk-transpose/walk-grace are otherwise
       verbatim -- none of them ever read the command's own leading
       keyword text, only find-child lookups by tag (multiply-factor/
       divide-factor/from-pitch/to-pitch/Sequence), which don't care
       whether the source was a backslash-keyword or a hidden Lisp-call
       literal. walk-grace's own backslash-stripping (str/replace #\"\\\\\" \"\")
       is a no-op on radical.ebnf's already-backslash-free grace-type
       words, so it needed no change either -- confirmed, not assumed."
...
(defn- extract-slur-marks
  "{:open? :close?} for this note's own SlurMark suffixes (LilyPond-style
   '(' / ')' glued directly onto a Note/Chord -- see radical.ebnf).
...
;; Variables (name = [ ... ] / \name)
;; ============================================================
;; Real grammar constructs (radical.ebnf's VarDef/VarRef, tag-identical
;; to musics.ebnf's own), resolved in the same single top-to-bottom walk
  ...
  (if (string? node)
    ...
    (let [tag      (first node)
      ...
      (case tag
        ...
        ;; case -- radical.ebnf produces none of those tags at all)
        ...
        ;; ---- Comments: real, tagged nodes (see radical.ebnf's ws/
        ...
        ;; string.
        :Comment     state
        ;; ---- Instructions ---- (no :Time/:Tempo/:Key -- radical.ebnf
        ...
        ;; ---- Commands ---- (no separate :tremolo case -- radical.ebnf's
        ;; own repeat rule covers unfold/volta/tremolo as one rule, see
        ;; walk-repeat below)
...
(defn- walk-multi-rest
  "R -- see radical.ebnf's own comment on MultiRest for the full
   LilyPond-superset story (identical to musics.ebnf's own): an explicit
   Duration is used literally, *n multiplying it. With no Duration at
...
(defn- walk-grace
  "grace/acciaccatura/appoggiatura/slashedGrace/afterGrace -- identical
   to flat-tree-walker's own walk-grace, INCLUDING its (str/replace
   #\"\\\\\" \"\") backslash-strip: radical.ebnf's own grace-type words
   (bare 'grace'/'acciaccatura'/.../'afterGrace' literals, no leading
   backslash -- see radical.ebnf's own header comment on hidden vs.
   visible command keywords) have no backslash to strip in the first
   place, so that call is a harmless no-op here rather than the
   meaningful strip it is against musics.ebnf's own \\grace-spelled
   tokens -- confirmed directly, not assumed, before leaving it
   unchanged."
  [state children]
...
(defn- walk-repeat
  "(repeat unfold/volta/tremolo N [body] (alternative [altbody])?) --
   radical.ebnf's own repeat rule covers all three repeat-types as one
   rule (a third repeat-type value, 'tremolo', alongside 'unfold'/
   'volta'), where musics.ebnf still has \\repeat unfold/volta N { ... }
   and a SEPARATE \\repeat tremolo N { ... } (walk-tremolo) sharing only
   the keyword. This absorbs flat-tree-walker's own walk-tremolo body as
   the 'tremolo' branch below, unchanged apart from field names --
   radical.ebnf names the count field `count` (not musics.ebnf's own
   `repeats`/`divisor`, two different names for what was really the same
   Int child) and the alternative-wrapper `alternative` (not `volta`).
   seq-node's own find-child :Sequence lookup is UNCHANGED from
   flat-tree-walker's own walk-repeat -- that function already only ever
   recognized a Sequence body, never the broader bare Element the old
   grammar's own repeat rule technically allowed (a known, documented
   gap between grammar and walker) -- so radical.ebnf requiring Sequence
   uniformly for all three types at the GRAMMAR level closes that gap
   without needing any change here at all."
  [state children]
...
(defn walk
  "Walk a raw instaparse tree (radical.ebnf's own) and build domain
