...
;; A tiny, deterministic factory: (fn [name a b] -> name), building a
;; wall fn that just stamps [a b] onto every node it sees, so a built
;; algo's own config is trivially observable without needing real
;; MIDI/playback.
(defn- stamp-factory [name a b]
...
(deftest two-built-algos-off-one-factory-stay-independent
  (with-fresh-registries
    ...
    (wall/build! ::bright ::stamp 1 2)
    (wall/build! ::dark   ::stamp 9 9)
...
(deftest build!-preserves-the-built-fns-own-doc
  (with-fresh-registries
    (wall/register-factory! ::stamp
      (fn [name a b] (wall/build-algo! name (fn [nodes _ctx _voice] (map #(assoc % :stamp [a b]) nodes))
...
(deftest unregistered-factory-name-warns-and-leaves-prior-build-untouched
  (with-fresh-registries
    (wall/register-factory! ::stamp stamp-factory)
    (wall/build! ::bright ::stamp 1 2)
    (with-out-str (wall/build! ::bright ::nonexistent 5 5))
...
(deftest build!-args-are-plain-literals-by-default
  (with-fresh-registries
    (wall/register-factory! ::stamp stamp-factory)
    (wall/build! ::p ::stamp [60 62 64] 1/4)
...
(deftest build!-resolves-groups-recursively-preserving-collection-type
  (with-fresh-registries
    ...
    (wall/register-factory! ::stamp stamp-factory)
    (wall/build! ::p ::stamp [:myColor :flag] #{1 :myColor})
...
(deftest build!-unwraps-a-real-parsed-duration-only-data-container
  (with-fresh-registries
    ...
    (let [{:keys [sid ids]} (m/parse "'[ /4 /8 /8 /4 ]")]
      (m/commit! sid)
      (wall/register-factory! ::stamp stamp-factory)
      (wall/build! ::p ::stamp (first ids) 0)
      (is (= [{:stamp [[1/4 1/8 1/8 1/4] 0]}] ((wall/algo ::p) [{}] [] nil))
