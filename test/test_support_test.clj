...
(deftest with-fresh-registries-is-genuinely-isolated-not-shared
  ...
  (wall/register-algo! ::leak-check (fn [nodes _ _] nodes))
  (with-fresh-registries
    (is (nil? (wall/algo-fn ::leak-check))
        "the outer registration is invisible inside a fresh binding"))
  (is (some? (wall/algo-fn ::leak-check))
...
(deftest with-fresh-registries-mutations-dont-escape-the-binding
  (with-fresh-registries
    (wall/register-algo! ::inside-only (fn [nodes _ _] nodes))
    (is (some? (wall/algo-fn ::inside-only))))
  (is (nil? (wall/algo-fn ::inside-only))
