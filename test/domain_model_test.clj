...
(deftest wire-context-leaf-replaces-context
  (let [parent-ctx (c/context-root {:amp 0.5})
        leaf (d/leaf "c4" (c/context) 1/4 [60])]
...
(deftest wire-context-composite-sets-parent
  (let [outer-ctx (c/context-root {:amp 0.5})
        inner (d/composite :SEQ "inner" (c/context-root {:tempo 120}))]
...
(deftest composite-empty
  (let [root (c/context-root {:tempo 120})
...
(deftest composite-with-children
  (let [root (c/context-root {:tempo 120})
        c (d/composite :SEQ "test" root
                       [(d/leaf "a" (c/context) 1/4 [60])
                        (d/leaf "b" (c/context) 1/4 [62])])]
...
(deftest composite-with-nested-composite
  (let [root (c/context-root {:tempo 120})
        inner-pt-ctx (c/context-root {:amp 0.5})
...
(deftest append-single-leaf
  (let [root (c/context-root {:tempo 120})
        ...
        leaf (d/leaf "a" (c/context) 1/4 [60])]
...
(deftest append-multiple-leaf
  (let [root (c/context-root {:tempo 120})
        c (d/composite :SEQ "phrase" root)]
    (d/composite-append c [(d/leaf "a" (c/context) 1/4 [60])
                            (d/leaf "b" (c/context) 1/4 [62])])
    (is (= 2 (d/composite-count c)))
...
(deftest append-composite
  (let [root (c/context-root {:tempo 120})
        outer (d/composite :SEQ "outer" root)
        inner (d/composite :SEQ "inner" (c/context-root {:amp 0.5}))]
...
(deftest insert-at-front
  (let [root (c/context-root {:tempo 120})
        c (d/composite :SEQ "phrase" root)]
    (d/composite-append c (d/leaf "a" (c/context) 1/4 [60]))
    (d/composite-insert c 0 (d/leaf "b" (c/context) 1/4 [62]))
...
(deftest insert-in-middle
  (let [root (c/context-root {:tempo 120})
        c (d/composite :SEQ "phrase" root)]
    (d/composite-append c [(d/leaf "a" (c/context) 1/4 [60])
                            (d/leaf "c" (c/context) 1/4 [64])])
    (d/composite-insert c 1 (d/leaf "b" (c/context) 1/4 [62]))
    (is (= ["a" "b" "c"] (map :id (d/composite-children c))))))

(deftest insert-composite
  (let [root (c/context-root {:tempo 120})
        outer (d/composite :SEQ "outer" root)]
    (d/composite-append outer (d/leaf "a" (c/context) 1/4 [60]))
    (d/composite-insert outer 0 (d/composite :SEQ "inner"
                                             (c/context-root {:amp 0.5})))
...
(deftest replace-returns-old
  (let [root (c/context-root {:tempo 120})
        old-leaf (d/leaf "old" (c/context) 1/4 [60])
        new-leaf (d/leaf "new" (c/context) 1/4 [62])
...
(deftest children-snapshot-immutable
  (let [c (d/composite :SEQ "test" (c/context-root {:tempo 120}))
...
(deftest count-progression
  (let [root (c/context-root {:tempo 120})
    ...
    (d/composite-append c (d/leaf "a" (c/context) 1/4 [60]))
    (is (= 1 (d/composite-count c)))
    (d/composite-append c [(d/leaf "b" (c/context) 1/4 [62])
                            (d/leaf "c" (c/context) 1/4 [64])])
...
(deftest score-root-only
  (let [score (d/make-score (c/context-root {:tempo 120}))]
    (is (= :SCORE (:type score)))
    (is (zero? (d/composite-count score)))))

(deftest score-with-part
  (let [score (d/make-score (c/context-root {:tempo 120})
                            (d/leaf "c4" (c/context) 1/4 [60]))]
...
(deftest leaf-under-nested-composite
  ...
  (let [root (c/context-root {:tempo 120})
        inner-root (c/context-root {:amp 0.5})
        leaf (d/leaf "c4" (c/context) 1/4 [60])
...
(deftest context-tree-after-multiple-appends
  ...
  (let [root (c/context-root {:tempo 120})
        ...
        section (d/composite :SEQ "verse" (c/context-root {:amp 0.6}))
        ;; save section's context BEFORE wiring into score
        section-ctx-original (:context section)
        na (d/leaf "c4" (c/context) 1/4 [60])
        nb (d/leaf "d4" (c/context) 1/4 [62])]
