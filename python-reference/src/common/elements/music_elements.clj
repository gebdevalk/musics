...
;;
;; Sections: Tempo, Meter, Pitch names, KeyScale, Chords, Circle of Fifths
...
(defn duration-seconds [^Tempo t dur] (quot (duration-ms t dur) 1000))

(defn tempo* [^Tempo t factor] (->Tempo (:duration t) (int (* (:bpm t) factor))))
(defn tempo+ [^Tempo t delta] (->Tempo (:duration t) (+ (:bpm t) delta)))

(defn tempo-
  ([^Tempo t delta] (->Tempo (:duration t) (- (:bpm t) delta)))
  ([^Tempo t1 ^Tempo t2] (->Tempo (:duration t1) (- (:bpm t1) (:bpm t2)))))
...
(defn name->pitch [name]
  (let [s      (str/lower-case (str/trim name))
        ...
        pc     (or (some #(when (= % pc-str) (second %))
                         (map-indexed vector data/note-names-sharp))
                   (some #(when (= % pc-str) (second %))
...
;; ============================================================
;; 4. KEYSCALE (key_scale.py)
;; ============================================================

(def scale-steps
  "Scale keyword → {:steps [step-sizes], :offset int}."
...
(defrecord KeyScale [key scale])

(defn keyscale
  "Create a KeyScale from key and scale keywords."
  [key-kw scale-kw]
  (let [k (get data/keys key-kw)
...
(defn keyscale-pitches [^KeyScale ks]
  (let [start (mod (+ (:tonic-pc (:key ks)) (:offset (:scale ks))) 12)]
    (loop [ps [start] steps (:steps (:scale ks)) cur start]
      (if (empty? steps) (vec (butlast ps))
          (let [nxt (+ cur (first steps))]
            (recur (conj ps nxt) (rest steps) nxt))))))

(defn keyscale-absolute [^KeyScale ks octave] (mapv #(+ % (* octave 12)) (keyscale-pitches ks)))

(defn keyscale-pitch-names [^KeyScale ks]
  (let [sharp? (>= (:accidental (:key ks)) 0)]
    (mapv #(pitch->name % sharp?) (keyscale-pitches ks))))

(defn keyscale->str [^KeyScale ks] (str (:display (:key ks)) "." (name (:scale ks))))
...
(def ^:private symbol->chord
  (reduce-kv (fn [m k v] (-> m (assoc (:symbol v) k)
                             (reduce #(assoc %1 %2 k) m (:aliases v))))
             {} chords))
...
(def ^:private cof-order [:Gb :Db :Ab :Eb :Bb :F :C :G :D :A :E :B :Fs])
...
(comment
  ...
  (def cm (keyscale :C :major))
  (keyscale-pitches cm)      ;; [0 2 4 5 7 9 11]
  (keyscale-pitch-names cm)  ;; ["c" "d" "e" "f" "g" "a" "b"]
