...
;;
;; Numeric defaults are sourced from common.data.defaults.ranges.
;;
...
(ns common.data.music-data
  (:require [common.data.defaults.ranges :as r]))
...
(def keys
  {:C  {:accidental 0  :tonic-pc 0  :display "C"}
   ...
   :Fs {:accidental 6  :tonic-pc 6  :display "F#" :alternative "Gb"}
...
;; ============================================================
;; 11. CONTEXT KEYS
;; Note: defaults are derived from common.data.defaults.ranges
...
(defn- reg!
  ...
  (let [rng   (when range-kw (r/ranges range-kw))
        dflt  (if range-kw (r/default range-kw) default')
        range' (when range-kw [(r/min-val range-kw) (r/max-val range-kw)])
        ck    {:name (name name)
               :type type
               :default dflt
...
(reg! :keyScale :any "Resolved KeyScale object"
