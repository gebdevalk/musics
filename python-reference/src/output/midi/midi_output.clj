...
(defn ^:private resolve-expressive
  ...
  (let [ctx (:context leaf)]
    ;; Volume: 0–100 domain → 0–127 MIDI
    (let [dyn   (or (:dynamic leaf)
                    (d/ctx-value ctx :volume time))
          vel   (-> dyn (* 1.27) Math/round (max 0) (min 127) int)]
      ;; Program (timbre)
...
(defn render-rest
  ...
  [^d/Rest rest ^el/Tempo tempo]
  (el/duration-seconds tempo (:duration rest)))

(defn render-drum
  "Render a Drum into a MidiDrumNote."
  [^d/Drum drum time ^el/Tempo tempo]
  (let [ctx (:context drum)
        dyn  (or (:dynamic drum) (d/ctx-value ctx :volume time) 50.0)
        vel  (-> dyn (* 1.27) Math/round (max 0) (min 127) int)
...
(defn musical->seconds
  ...
  [^d/Context ctx offset & {:keys [step] :or {step 1/64}}]
  (loop [t 0 secs 0.0]
    (if (>= t offset)
      secs
...
(defn compute-onset
  ...
  [start-time ^d/Context ctx offset micro-on]
