...
from archive.old.domain.container import Concurrent, Algorithm  # your parallel container
...
class MidiEngineAsync:
    ...
    async def perform(self, channel: Channel, part, start_time: float):
        ...
        if isinstance(part, Leaf):
            ...
            micro_on = compute_micro_on(ctx, time_f, time_Fraction)
            onset_time = compute_onset(start_time, ctx, time_Fraction, micro_on)

            note = render_leaf(part, time_Fraction, channel.number)

            await sleep_until(onset_time)
            await self.play_note(channel, note, onset_time, ctx, time_Fraction)
        ...
        if isinstance(part, DrumLeaf):
            ...
            await sleep_until(onset_time)
            await self.play_drum_note(drum_note, onset_time, ctx, time_Fraction)
    ...
    async def play_note(
        self, channel: Channel, note: MidiNote, onset_time: float, ctx, time_Fraction: Fraction
    ):
        ...
        if not channel.sounding(note.pitches):
            if not note.tied:  # normal note
                ...
                await self.create_note_off_task(channel, note, onset_time, ctx, time_Fraction)
    ...
    async def play_drum_note(
        self, note: MidiDrumNote, onset_time: float, ctx, time_Fraction: Fraction
    ):
        self.midi_out.send(mido.Message(
            'note_on', channel=DRUM_CHANNEL, note=note.timbre, velocity=note.velocity
        ))
        time_f = float(time_Fraction)
        micro_off = compute_micro_off(ctx, time_f, time_Fraction)
        noteoff_time = compute_drum_noteoff_time(onset_time, note.duFractionn_played, micro_off)
    ...
    async def create_note_off_task(
        ...
        self.tasks.append(
            asyncio.create_task(
                self.note_off_after_delay(channel, note, onset_time, ctx, time_Fraction)
            )
        )
    ...
    async def note_off_after_delay(
        ...
        time_f = float(time_Fraction)
        micro_off = compute_micro_off(ctx, time_f, time_Fraction)
        target_time = compute_noteoff_time(onset_time, note.duFractionn_played, micro_off)
