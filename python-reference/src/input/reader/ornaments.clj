...
;; Applied at resolution time (needs KeyScale from context).
...
(defn- scale-neighbor
  ...
  [^el/KeyScale ks pitch dir]
  (let [pc (mod pitch 12)
        octave (quot pitch 12)
        scale-pcs (el/keyscale-pitches ks)]
...
;; ============================================================
;; Ornament functions — Leaf + KeyScale → [Leaf ...]
;; ============================================================

(defn plain [leaf _] [leaf])

(defn prall [^d/Leaf leaf ks]
...
(defn prallprall [^d/Leaf leaf ks]
...
(defn prallmordent [^d/Leaf leaf ks]
  (let [d8 (/ (:duration leaf) 8) ctx (:context leaf)
        art (:articulation leaf) dyn (:dynamic leaf)
        p (:pitches leaf) hi (mapv #(upper ks %) p) lo (mapv #(lower ks %) p)]
    [(d/leaf "" ctx d8 hi art dyn nil false) (d/leaf "" ctx d8 p art dyn nil false)
...
(defn downmordent [^d/Leaf leaf ks]
...
(defn reverseturn [^d/Leaf leaf ks]
  (let [d4 (/ (:duration leaf) 4) ctx (:context leaf)
        art (:articulation leaf) dyn (:dynamic leaf) p (:pitches leaf)]
    [(d/leaf "" ctx d4 (mapv #(lower ks %) p) art dyn nil false)
     (d/leaf "" ctx d4 p art dyn nil false)
...
(defn expand
  "Expand leaf's ornament into sub-leaves. Looks up KeyScale from context.
